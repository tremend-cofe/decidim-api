...
module Decidim
  module Core
    describe ComponentInputSort, type: :graphql do
      ...
      let!(:proposal) { create(:dummy_component, :published, manifest_name: :proposals, participatory_space: model, name: generate_component_name(current_organization.available_locales, :dummy)) }
      let!(:dummy) { create(:dummy_component, :published, participatory_space: model) }
